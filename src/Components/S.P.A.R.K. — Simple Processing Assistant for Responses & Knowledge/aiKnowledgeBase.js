...
export function generateResponse(userMessage, personality = { professionalism: 0.5, mentorship: 0.5 }) {
  ...
  if (/^(hi|hello|hey|yo|sup|hiya)\b/.test(lower)) {
    return buildResponse(
      "Hey there! I'm here to help with studying, writing, coding, math, and Nexus features.",
      "Hi!",
      personality
    );
  }
...
function buildResponse(content, intro, personality, isBoundary = false) {
  ...
  // Adjust intro based on professionalism
  if (professionalism < 0.3) {
    // Moody intro
    const moodyIntros = [
  ...
  // Add personality flair (moody → kaomoji/casual)
  if (professionalism < 0.3 && Math.random() > 0.5 && !isBoundary) {
    const kaomojis = ['(´｀)', '( ´∀`)', '(๑•́ ω •̀๑)', '¯\\_(ツ)_/¯'];
