...
async function resolveUserSettings(userSettings = {}) {
  try {
    const settings = await storage.loadSettings();
    const aiTools = settings?.aiTools || {};
    const provider = userSettings.apiProvider ?? aiTools.apiProvider ?? 'none';
    const providerKey = userSettings.apiKey ?? aiTools.apiKey ?? '';

    return {
      ...userSettings,
      apiProvider: provider,
      model: userSettings.model ?? aiTools.model,
      serpApiKey: userSettings.serpApiKey ?? aiTools.serpApiKey,
      openaiKey: userSettings.openaiKey ?? userSettings.openai ?? (provider === 'openai' ? providerKey : ''),
      googleKey: userSettings.googleKey ?? userSettings.google ?? (provider === 'google' ? providerKey : ''),
      anthropicKey: userSettings.anthropicKey ?? userSettings.anthropic ?? (provider === 'anthropic' ? providerKey : ''),
    };
  } catch (err) {
    return userSettings;
  }
...
export async function generateAASResponse(userMessage, options = {}) {
  ...
  try {
    ...
    const resolvedUserSettings = await resolveUserSettings(userSettings);

    // Step 2: Route the question (LOCAL vs API)
    const strategy = routeQuestion(userMessage, resolvedUserSettings);
    ...
    if (strategy.route[0] === 'LOCAL' && generateLocal) {
      ...
      const resolvedApiKeys = {
        ...apiKeys,
        apiProvider: resolvedUserSettings.apiProvider,
        apiKey: resolvedUserSettings.apiProvider === 'openai'
          ? resolvedUserSettings.openaiKey
          : resolvedUserSettings.apiProvider === 'google'
            ? resolvedUserSettings.googleKey
            : resolvedUserSettings.apiProvider === 'anthropic'
              ? resolvedUserSettings.anthropicKey
              : undefined,
        openaiKey: apiKeys.openaiKey ?? resolvedUserSettings.openaiKey,
        googleKey: apiKeys.googleKey ?? resolvedUserSettings.googleKey,
        anthropicKey: apiKeys.anthropicKey ?? resolvedUserSettings.anthropicKey,
        model: apiKeys.model ?? resolvedUserSettings.model,
        serpApiKey: apiKeys.serpApiKey ?? resolvedUserSettings.serpApiKey,
      };
