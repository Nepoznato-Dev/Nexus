...
import { callGoogleGemini } from './aiApiBridge.js';
...
Format your response clearly with emojis and make it easy to read.`;
  ...
  try {
    const result = await callGoogleGemini(searchPrompt, apiKey, [], {
      enableSearch: true,
      searchQuery: query,
    });
...
Use emojis to make it engaging! Format clearly with proper math notation.`;
  ...
  try {
    const result = await callGoogleGemini(mathPrompt, apiKey);
...
Make it engaging with emojis and clear formatting!`;
  ...
  try {
    const result = await callGoogleGemini(infoPrompt, apiKey);
...
Focus on educational, official, or well-known sources.`;
  ...
  try {
    const result = await callGoogleGemini(linksPrompt, apiKey);
