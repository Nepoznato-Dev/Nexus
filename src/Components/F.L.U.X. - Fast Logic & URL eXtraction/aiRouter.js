...
export const explainModelChoice = (question, chosenModel, complexity) => {
  ...
  if (complexity.complexity <= 2) {
    ...
    if (chosenModel === 'GOOGLE') {
      reasons.push('Google Gemini provides good quality at no cost for medium complexity');
    }
  } else {
    reasons.push('Question is complex and requires advanced reasoning');
    if (chosenModel === 'OPENAI') {
      reasons.push('OpenAI GPT excels at complex explanations and nuanced answers');
    } else if (chosenModel === 'GOOGLE') {
      reasons.push('Using Google Gemini (best available option)');
    }
  }

  // Model-specific benefits
  const benefits = {
    'OPENAI': ['Best for complex reasoning', 'Excellent at code and math', 'Nuanced explanations'],
    'GOOGLE': ['Free tier available', 'Fast responses', 'Good general knowledge'],
    'ANTHROPIC': ['Very thoughtful responses', 'Strong at analysis', 'Safe and helpful'],
    'LOCAL': ['Instant responses', 'Zero API cost', 'Privacy-focused'],
  };

  if (benefits[chosenModel]) {
    reasons.push(`Benefits: ${benefits[chosenModel].join(', ')}`);
  }
