...
import { pushPendingUndo } from './irisPendingUndo.js';
...
/**
 * Bind an action to an element, replacing the current binding if present.
 * Automatically creates a pending Keep/Undo entry for AI-sourced changes.
 */
export async function bindActionToElement(elementId, actionName, parameters = {}, options = {}) {
  const {
    source = 'AI',
    expiresMs = 30000,
    withUndo = true,
  ...
  if (!elementId || !actionName) {
    return { success: false, error: 'elementId and actionName are required' };
  }
  ...
  try {
    ...
    const index = settings.actionBindings.findIndex((b) => b.elementId === elementId);
    const previousBinding = index >= 0 ? { ...settings.actionBindings[index] } : null;

    const newBinding = new ActionBinding(elementId, actionName, parameters, bindingOptions).export();

    // Preserve stable binding id when replacing existing element binding.
    if (previousBinding?.id) {
      newBinding.id = previousBinding.id;
    }

    if (index >= 0) {
      settings.actionBindings[index] = newBinding;
    } else {
      settings.actionBindings.push(newBinding);
    }
    ...
    if (withUndo) {
      await pushPendingUndo({
        summary: `Bound "${elementId}" to "${actionName}". Keep this change?`,
        undo: async () => {
          const currentSettings = await storage.loadSettings();
          if (!currentSettings.actionBindings) {
            currentSettings.actionBindings = [];
          }

          const currentIndex = currentSettings.actionBindings.findIndex((b) => b.elementId === elementId);
          const currentBinding = currentIndex >= 0 ? currentSettings.actionBindings[currentIndex] : null;

          // Prevent accidental overwrite of a newer manual update.
          const stillApplied = currentBinding &&
            currentBinding.id === newBinding.id &&
            currentBinding.actionName === newBinding.actionName &&
            JSON.stringify(currentBinding.parameters || {}) === JSON.stringify(newBinding.parameters || {});

          if (!stillApplied) {
            throw new Error('Binding changed after this action; undo skipped to avoid overwriting newer edits.');
          }

          if (previousBinding) {
            currentSettings.actionBindings[currentIndex] = previousBinding;
          } else {
            currentSettings.actionBindings.splice(currentIndex, 1);
          }

          await storage.saveSettings(currentSettings);
        },
        keep: async () => {
          // Keep is intentional no-op; current saved binding remains final.
        },
        expiresMs,
      });
    }
    ...
    return {
      success: true,
      source,
      replaced: !!previousBinding,
      elementId,
      actionName,
      binding: newBinding,
    };
  } catch (error) {
    console.error('Error binding action to element:', error);
    return { success: false, error: error.message };
  }
}
...
if (typeof window !== 'undefined') {
  window.irisBindActionToElement = bindActionToElement;
}
