...
export class IRISPerformanceManager {
  constructor() {
    ...
    this.userPreferences = {
      ...
      allowSelfCull: false,
      allowSparkFallbackCull: true,
      sparkRecoveryDelayMs: 10000,
      customMessages: {
  ...
  async checkPerformance() {
    ...
    if (this.userPreferences.autoOptimize) {
      if (this.safeMode) {
        if (this.userPreferences.allowSparkFallbackCull) {
          await this.runSparkFallbackCulling(usage);
        } else {
  ...
  /**
   * Clear runtime caches and request a UI-only reload
   */
  async clearRamAndReloadUI() {
    const removableLocalStorageKeys = [];
    for (let index = 0; index < localStorage.length; index += 1) {
      const key = localStorage.key(index);
      if (!key) continue;
      const lowerKey = key.toLowerCase();
      const isPreloadKey = lowerKey.includes('cache') || lowerKey.includes('prefetch') || lowerKey.includes('preload');
      const keepKey = lowerKey.includes('nexus_settings') || lowerKey.includes('nexus_user') || lowerKey.includes('nexus_session');
      if (isPreloadKey && !keepKey) {
  ...
  /**
   * Lightweight fallback culling while I.R.I.S AI is self-culled
   * Keeps memory pressure down, and wakes AI once performance is stable
   */
  async runSparkFallbackCulling(usage) {
    const now = Date.now();
    const thresholds = {
      low: { ram: 85, cpu: 85 },
      medium: { ram: 75, cpu: 75 },
    ...
    const current = thresholds[this.userPreferences.aggressiveness] || thresholds.medium;
    const stableRam = Math.max(35, current.ram - 8);
    const stableCpu = Math.max(35, current.cpu - 8);
    const isStable = usage.ram.percentage <= stableRam && usage.cpu.estimated <= stableCpu;

    if (!this.sparkFallbackState.active) {
      this.activateSparkFallback();
    }

    if (isStable) {
      if (!this.sparkFallbackState.stableSince) {
        this.sparkFallbackState.stableSince = now;
      }

      const stableForMs = now - this.sparkFallbackState.stableSince;
      if (stableForMs >= this.userPreferences.sparkRecoveryDelayMs && this.suspendedTasks.has('ai-thinking')) {
        await this.resumeProcess('ai-thinking');
      }
    } else {
      this.sparkFallbackState.stableSince = null;
    }

    if (now - this.sparkFallbackState.lastCullAt < this.sparkFallbackState.cullCooldownMs) {
      return;
    }
    ...
    const candidates = usage.processes.filter(p =>
      ...
      !this.userPreferences.protectedApps.includes(p.name) &&
      p.type !== 'ai' &&
      (!p.active || this.userPreferences.allowActiveCull)
    );
    ...
    const target = candidates[0];
    await this.suspendProcess(target);
    this.sparkFallbackState.lastCullAt = now;
  }

  activateSparkFallback() {
    this.sparkFallbackState.active = true;
    this.sparkFallbackState.stableSince = null;
  }

  deactivateSparkFallback() {
    this.sparkFallbackState.active = false;
    this.sparkFallbackState.stableSince = null;
  }
